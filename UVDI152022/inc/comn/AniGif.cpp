//////////////////////////////////////////////////////////////////////
// PictureEx.cpp: implementation of the CAniGif class.
//
...
// Copyright (c) 2001
//
// To use CAniGif, follow these steps:
...
//   - in your dialog's header file replace CStatic with CAniGif
//     (don't forget to #include "PictureEx.h" and add 
//     PictureEx.h and PictureEx.cpp to your project)
//   - call one of the overloaded CAniGif::Load() functions somewhere
//     (OnInitDialog is a good place to start)
//   - if the preceding Load() succeeded call Play()
//  
// You can also add the control by defining a member variable of type 
// CAniGif, calling CAniGif::Create (derived from CStatic), then 
// CAniGif::Load and CAniGif::Play.
//
// By default, the control initializes its background to COLOR_3DFACE
// (see CAniGif::PrepareDC()). You can change the background by
// calling CAniGif::SetBkColor(COLORREF) after CAniGif::Load().
//
...
// and an appropriate section in CAniGif::Load(HGLOBAL, DWORD). These functions 
...
//                Now CAniGif[Wnd] works in Win95 too.
...
//                CAniGifWnd::PrepareDC() function).
//
//              - Now it is possible to set a clipping rectangle using
//                CAniGif[Wnd]::SetPaintRect(const LPRECT) function.
...
//                animated GIF, then Play() it again, it will continue
...
inline int CAniGif::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
{
...
inline int CAniGif::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
inline int CAniGif::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
BEGIN_MESSAGE_MAP(CAniGif, CStatic)
	//{{AFX_MSG_MAP(CAniGif)
	ON_WM_DESTROY()
	ON_WM_PAINT()
	//}}AFX_MSG_MAP
...
BOOL CAniGif::Load(HGLOBAL hGlobal, DWORD dwSize)
{
...
BOOL CAniGif::Play()
{
	if (!m_bIsInitialized)
	{
		TRACE(_T("Call one of the CAniGif::Load() member functions before calling Play()\n"));
		return FALSE;
	};
...
SIZE CAniGif::GetSize() const
{
	return m_PictureSize;
}

BOOL CAniGif::Load(LPCTSTR szFileName, BOOL ready)
{
	ASSERT(szFileName);
...
BOOL CAniGif::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
...
BOOL CAniGif::SkipNextGraphicBlock()
{
...
UINT CAniGif::GetSubBlocksLen(UINT nStartingOffset) const
{
	UINT nRet = 0;
...
enum CAniGif::GIFBlockTypes CAniGif::GetNextBlock() const
{
...
int CAniGif::GetNextBlockLen() const
{
...
UINT WINAPI CAniGif::_ThreadAnimation(LPVOID pParam)
{
	ASSERT(pParam);
	CAniGif *pPic = reinterpret_cast<CAniGif *> (pParam);
...
HGLOBAL CAniGif::GetNextGraphicBlock(UINT *pBlockLen, 
...
BOOL CAniGif::IsGIF() const
{
	return m_bIsGIF;
}

BOOL CAniGif::IsAnimatedGIF() const
{
	return (m_bIsGIF && (m_arrFrames.size() > 1));
}

BOOL CAniGif::IsPlaying() const
{
	return m_bIsPlaying;
}

int CAniGif::GetFrameCount() const
{
	if (!IsAnimatedGIF())
		return 0;
...
COLORREF CAniGif::GetBkColor() const
{
	return m_clrBackground;
}

void CAniGif::OnPaint() 
{
...
BOOL CAniGif::PrepareDC(int nWidth, int nHeight)
{
...
void CAniGif::OnDestroy() 
{
	Stop();	
	CStatic::OnDestroy();
}

void CAniGif::SetBkColor(COLORREF clr)
{
...
#ifdef GIF_TRACING
void CAniGif::WriteDataOnDisk(CString szFileName, HGLOBAL hData, DWORD dwSize)
{
	CFile file;
...
BOOL CAniGif::SetPaintRect(const RECT *lpRect)
{
	return CopyRect(&m_PaintRect, lpRect);
}

BOOL CAniGif::GetPaintRect(RECT *lpRect)
{
