...
BOOL CDirTreeCtrl::DisplayDrives()
{
	...
	if (!GetLogicalDriveStrings(sizeof(tzDrives), tzDrives))
	{
	...
	while (*pDrive)
	{
		...
		if (FindSubDir(pDrive))	InsertItem(L"", 0, 0, hParent);
...
void CDirTreeCtrl::DisplayPath(HTREEITEM hParent, LPCTSTR strPath)
{
	...
	for (i=0; i<strDirArray.GetSize(); i++)
	{
		...
		if (FindSubDir(strDirArray.GetAt(i)))	InsertItem(L"", 0, 0, hItem);
	}
...
HTREEITEM CDirTreeCtrl::AddItem(HTREEITEM hParent, LPCTSTR strPath)
{
	...
	if (strTemp.Right(1) == L"\\")	strTemp.SetAt(strTemp.GetLength() - 1, L'\0');
...
BOOL CDirTreeCtrl::SetSelPath(LPCTSTR strPath)
{
	...
	INT32 i32Len	= (INT32)_tcslen(strPath) + 2;
	PTCHAR ptzPath	= new TCHAR[i32Len];
	PTCHAR pPath	= ptzPath;
	BOOL bRet		= FALSE;
    
	if (!IsValidPath( strPath))
	{
		delete [] ptzPath; // this must be added 29.03.99
		return FALSE;
	}
		
	wcscpy_s(ptzPath, i32Len, strPath);
	_wcsupr_s(ptzPath, i32Len);
	
	if (ptzPath[_tcslen(ptzPath)-1] != L'\\')
		wcscat_s(ptzPath, i32Len, L"\\");
    
	for (UINT16 i=0; i<(UINT16)_tcslen(ptzPath); i++)
	{
		if (ptzPath[i] == L'\\')
		{
			SetRedraw( FALSE);
