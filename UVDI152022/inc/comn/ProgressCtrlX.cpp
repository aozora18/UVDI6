...
void CProgressCtrlX::OnPaint() 
{
	...
	if (0 == (info.nUpper - info.nLower))
	{
		rcBar.right = rcMax.right;//(int)((float)(info.nCurPos - info.nLower) * rcMax.right / (info.nUpper - info.nLower));
	}
	else
	{
...
void CProgressCtrlX::DrawText(const CDrawInfo& info, const CRect &rcMax, const CRect &rcBar)
{
  ...
  CPoint ptOrg = pDC->GetWindowOrg();
	...
	{
		sFormat = "%d%%\n%s";
		sText.Format(sFormat, nValue, m_strTestText);
	}
	else
	{
		sFormat = "%d%%";
...
{
	SetGradientColors((COLORREF)clrStart, clrEnd ? (COLORREF)clrEnd : (COLORREF)clrStart);
