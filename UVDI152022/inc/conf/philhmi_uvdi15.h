...
enum __en_phiihmi_error_code__
{
	...
	ePHILHMI_ERR_DI_MC2_NOT_CONNECTED			= 60104,	// MC2 연결되지 않음
	ePHILHMI_ERR_DI_MC2_DRIVE_ERROR				= 60105,	// MC2 드라이브 에러 발생
	ePHILHMI_ERR_DI_MC2_DNC_ERROR				= 60106,	// MC2 DNC 에러 발생
	...
	ePHILHMI_ERR_DI_CMD_MOVE_STAGE_X			= 60204,	// SendDevAbsMoveStageX 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_MOVE_STAGE_Y			= 60205,	// SendDevAbsMoveStageY 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_MOVE_ACAM1				= 60206,	// SendDevAbsMoveACam1 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_MOVE_ACAM2				= 60207,	// SendDevAbsMoveACamX2 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_TUNE_REF_WRITE			= 60208,	// savEng_Tune_RefSetWrite 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_LED_AMPLITUDE		= 60209,	// ReqSetLedAmplitude 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_DEV_DEFAULT_RESET		= 60210,	// SendDevDefaultReset 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_DEV_HOMING_ALL			= 60211,	// SendDevHomingAll 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_FAULT_RESET_ALL			= 60212,	// FaultResetAll 명령 전송 실패

	/* ALIGN & MARK (60300 ~) */
	ePHILHMI_ERR_DI_ALIGN_MARK_GET_POS_FAIL		= 60301,	// 마크 측정을 위한 이동 좌표 획득 실패
	ePHILHMI_ERR_DI_ALIGN_MARK_LOC_INFO_FAIL	= 60302,	// 마크 위치 정보 획득 실패
	ePHILHMI_ERR_DI_ALIGN_MARK_WIDTH_PERIOD		= 60303,	// 마크 간 너비 주기가 카메라 주기보다 작음
	ePHILHMI_ERR_DI_ALIGN_MARK_HEIGHT_PERIOD	= 60304,	// 마크 간 높이 주기가 카메라 주기보다 작음

	/* GERBER & RECIPE (60400 ~) */
	ePHILHMI_ERR_DI_RECIPE_NO_THICKNESS_DATA	= 60401,	// 레시피에 자재 두께 보정 데이터 없음
	ePHILHMI_ERR_DI_RECIPE_NO_THICKNESS_INFO	= 60402,	// 해당 레시피의 두께 보정 정보 없음
	...
	/*70000~ Alarm 처리*/
	/* MOTION & INTERLOCK (70100 ~) */
	ePHILHMI_ERR_DI_MOVE_ALIGN_CAM1_LEFT		= 70101,	// 얼라인 카메라1 좌측 이동 실패
	ePHILHMI_ERR_DI_MOVE_ALIGN_CAM2_LEFT		= 70102,	// 얼라인 카메라2 좌측 이동 실패
	ePHILHMI_ERR_DI_MOVE_ALIGN_CAM1_RIGHT		= 70103,	// 얼라인 카메라1 우측 이동 실패
	ePHILHMI_ERR_DI_MOVE_ALIGN_CAM2_RIGHT		= 70104,	// 얼라인 카메라2 우측 이동 실패
	ePHILHMI_ERR_DI_HOME_ALIGN_CAM1_LEFT		= 70105,	// 얼라인 카메라1 좌측 홈 이동 실패
	ePHILHMI_ERR_DI_HOME_ALIGN_CAM2_LEFT		= 70106,	// 얼라인 카메라2 좌측 홈 이동 실패
	ePHILHMI_ERR_DI_MOVE_PHOTOHEAD_Z_LIMIT		= 70107,	// 포토헤드 Z축 리밋 이동 실패
	ePHILHMI_ERR_DI_MOVE_PHOTOHEAD_Z_POS		= 70108,	// 포토헤드 Z축 지정 위치 이동 실패
	ePHILHMI_ERR_DI_OPTICAL_Z_OUT_OF_RANGE		= 70109,	// 광학 Z축 이동 범위 초과
	ePHILHMI_ERR_DI_UNKNOWN_VECTOR_DRIVE		= 70110,	// 알 수 없는 벡터 드라이브 이동 실패
	/* COMMAND (CMD) SEND/RECV (70200 ~) */
	ePHILHMI_ERR_DI_CMD_SET_EXPOSURE_XY			= 70201,	// ReqSetExposureStarXY 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_ALL_PHOTOHEADS		= 70202,	// ReqSetAllPhotoheads 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_PHOTOHEAD_OFFSET	= 70203,	// ReqSetSpecPhotoHeadOffset 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_MOTION_CONTROL		= 70204,	// ReqSetMotionControl 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_MOTION_TYPE			= 70205,	// ReqSetMotionType1 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_TABLE_SETTING		= 70206,	// ReqSetTableSetting 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_TABLE_LIMIT			= 70207,	// ReqSetTablePositionLimit 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_UPDATE_PHOTOHEADS		= 70208,	// ReqSetUpdatePhotoheads 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_SYSTEM_SETTINGS		= 70209,	// ReqSetSystemSettings 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_GET_JOB_LIST			= 70210,	// ReqGetJobList 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SELECT_JOB_NAME			= 70211,	// ReqSelectJobName 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_LOAD_JOB				= 70212,	// ReqSelectLoadSelectJob 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_SELECT_JOB_INDEX	= 70213,	// ReqSetSelectedJobIndex 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_LOCAL_ZONE			= 70214,	// ReqSetSharedLocalZone 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_REGIS				= 70215,	// ReqSetRegis 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_GET_REGIS_STATUS		= 70216,	// ReqGetRegisStatus 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_MOTOR_ABS_POS		= 70217,	// ReqSetMotorAbsPositionAll 명령 전송 실패
	ePHILHMI_ERR_DI_CMD_SET_EXPOSURE_FACTOR		= 70218,	// ReqSetExposureFactor 명령 전송 실패
	/* ALIGN & MARK (70300 ~) */
	ePHILHMI_ERR_DI_ALIGN_MARK_COUNT_GLOBAL		= 70301,	// 글로벌 마크 개수 오류
	ePHILHMI_ERR_DI_ALIGN_MARK_NO_REGISTERED	= 70302,	// 카메라에 등록된 얼라인 마크 없음
	ePHILHMI_ERR_DI_ALIGN_MARK_DIST_INVALID		= 70303,	// 측정된 마크 간 거리 유효하지 않음
	ePHILHMI_ERR_DI_ALIGN_MARK_HV_ERROR			= 70304,	// 마크 가로/세로 거리 오차 발생
	ePHILHMI_ERR_DI_ALIGN_MARK_ALL_INVALID		= 70305,	// 발견된 모든 마크가 유효하지 않음
	ePHILHMI_ERR_DI_ALIGN_OFFSET_GLOBAL_FAIL	= 70306,	// 글로벌 마크 노광 오프셋 획득 실패
	ePHILHMI_ERR_DI_ALIGN_OFFSET_LOCAL_FAIL		= 70307,	// 로컬 마크 노광 오프셋 획득 실패
	ePHILHMI_ERR_DI_INSPECT_ALL_MARKS_FAIL		= 70308,	// 전체 마크 검사 실패
	/* GERBER & RECIPE (70400 ~) */
	ePHILHMI_ERR_DI_GERBER_GLOBAL_MARK_FAIL		= 70401,	// 선택된 거버에서 글로벌 마크 획득 실패
	ePHILHMI_ERR_DI_GERBER_LOCAL_MARK_FAIL		= 70402,	// 선택된 거버에서 로컬 마크 획득 실패
	ePHILHMI_ERR_DI_GERBER_FILE_NOT_EXIST		= 70403,	// 거버 파일이 존재하지 않음
	ePHILHMI_ERR_DI_GERBER_LOAD_TIMEOUT			= 70404,	// 거버 로딩 시간 초과
	ePHILHMI_ERR_DI_GERBER_LOAD_INCOMPLETE		= 70405,	// 거버 로딩 미완료
	ePHILHMI_ERR_DI_GERBER_REGIST_FAIL			= 70406,	// 거버 등록 실패
	...
	ePHILHMI_ERR_DI_MODEL_FIND_GLOBAL_FAIL		= 70503,	// 글로벌 마크 모델 찾기 실패
	ePHILHMI_ERR_DI_MODEL_FIND_LOCAL_FAIL		= 70504,	// 로컬 마크 모델 찾기 실패
	ePHILHMI_ERR_DI_IMAGE_CAPTURE_TIMEOUT		= 70505,	// 이미지 캡처 대기 시간 초과
	ePHILHMI_ERR_DI_BIG_DATA_STAGE_Y_EMPTY		= 70506,	// Cail Data (Stage_Y) 호출 실패
	ePHILHMI_ERR_DI_BIG_DATA_GLOBAL_Y_EMPTY		= 70507,	// Cail Data (Global_Y) 호출 실패
	ePHILHMI_ERR_DI_BIG_DATA_LOCAL_Y_EMPTY		= 70508,	// Cail Data (Local_Y) 호출 실패
	/* SYSTEM & PROCESS (70600 ~) */
