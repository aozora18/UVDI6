...
//pja - Sept 12, 2004
//  OnChar, OnKeyDown, and OnKeyUp should return FALSE if the cell
//  wants the grid control to do it's default handling of the messages,
//  and return TRUE if it wants to block further processing.
BOOL CGridCellBase::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	UNUSED_ALWAYS(nChar);
	UNUSED_ALWAYS(nRepCnt);
	UNUSED_ALWAYS(nFlags);
	TRACE("CGridCellBase::OnChar\n");

	return FALSE;
}

BOOL CGridCellBase::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	UNUSED_ALWAYS(nChar);
	UNUSED_ALWAYS(nRepCnt);
	UNUSED_ALWAYS(nFlags);
	TRACE("CGridCellBase::OnKeyDown\n");

	return FALSE;
}

BOOL CGridCellBase::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	UNUSED_ALWAYS(nChar);
	UNUSED_ALWAYS(nRepCnt);
	UNUSED_ALWAYS(nFlags);
	TRACE("CGridCellBase::OnKeyUp\n");
