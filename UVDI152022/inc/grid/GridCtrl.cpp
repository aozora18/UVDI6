...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_crShadow			  = WHITE_;//::GetSysColor(COLOR_3DSHADOW);
    m_crGridLineColour    = SLATE_GRAY;//RGB(192,192,192);
    ...
    // LUC
	m_nTimerInterval      = 50;         // (in milliseconds)
	...
	m_bDrawScrollBarHorz = FALSE;		// 기본적으로 수평 스크롤바 그려줌
	m_bDrawScrollBarVert = FALSE;		// 기본적으로 수직 스크롤바 그려줌
...
// 20230530 mhbaek Add
void CGridCtrl::SetScrollCell(int nRow, int nCol, int nMoveToRow, int nMoveToCol)
{
	CRect rect;
	CCellID idTopLeft = GetTopleftNonFixedCell();
	int nScrollX = GetRowHeight(idTopLeft.col);
	int nScrollY = GetRowHeight(idTopLeft.row);

	GetClientRect(rect);

	nScrollX = (nCol - nMoveToCol < 0) ? 0 : nScrollX * (nCol - nMoveToCol);
	nScrollY = (nRow - nMoveToRow < 0) ? 0 : nScrollY * (nRow - nMoveToRow);

	SetScrollPos32(SB_HORZ, nScrollX);
	SetScrollPos32(SB_VERT, nScrollY);

	InvalidateRect(rect);
}

void CGridCtrl::SetCellView(int nRow, int nCol)
{
	CRect rect;
	CCellID idTopLeft = GetTopleftNonFixedCell();
	int nCellPosX = GetColumnWidth(idTopLeft.col);
	int nCellPosY = GetRowHeight(idTopLeft.row);
	int nCurScrollPosX = GetScrollPos32(SB_HORZ);
	int nCurScrollPosY = GetScrollPos32(SB_VERT);
	int nMaxRowCnt = 0;
	int nMaxColCnt = 0;

	GetClientRect(rect);

    if (0 != rect.Width() && 0 != nCellPosX)
    {
        nMaxColCnt = rect.Width() / nCellPosX;
        nMaxColCnt -= (rect.Width() % nCellPosX) ? 1 : 0;
    }
    if (0 != rect.Height() && 0 != nCellPosY)
    {
		nMaxRowCnt = rect.Height() / nCellPosY;
        nMaxRowCnt -= (rect.Height() % nCellPosY) ? 1 : 0;
    }

    if (nCurScrollPosX > nCellPosX * nCol)
	{
		SetScrollPos32(SB_HORZ, nCellPosX * nCol);
    }
	else if (nCurScrollPosX < nCellPosX * (nCol - nMaxColCnt))
	{
		SetScrollPos32(SB_HORZ, nCellPosX * (nCol - nMaxColCnt));
	}

	if (nCurScrollPosY > nCellPosY * nRow)
	{
		SetScrollPos32(SB_VERT, nCellPosY * nRow);
	}
	else if (nCurScrollPosY < nCellPosY * (nRow - nMaxRowCnt))
	{
		SetScrollPos32(SB_VERT, nCellPosY * (nRow - nMaxRowCnt));
	}
...
BOOL CGridCtrl::SetItemUILevel(int nRow, int nCol, UINT level)
{
    if (level <= g_loginLevel || eLOGIN_LEVEL_UNKNOWN == g_loginLevel)
    {
        SetItemState(nRow, nCol, GetItemState(nRow, nCol) | GVIS_FIXED | GVIS_FIXEDROW);
        m_nFixedRows = nRow;
    }
...
//************************************
// Method: SetItemTextDes
// Date: 2016/08/03
// Author: JAEYOON, JEONG
// Returns: BOOL
// Parameter: int nRow
// Parameter: int nCol
// Parameter: LPCTSTR str
// Comment:설명 표시
//************************************
BOOL CGridCtrl::SetItemTextDes(int nRow, int nCol, LPCTSTR str)
{
	if (GetVirtualMode())
		return FALSE;
...
// Parameter: int nCol
// Parameter: BOOL bUse
// Comment: Cell의 LanguageMgr 사용 여부 결정
//************************************
void CGridCtrl::SetItemTextTranslation(int nRow, int nCol, BOOL bUse)
{
	if (GetVirtualMode())
		return;
...
//						TRACE0("Showing TitleTip\n");
						m_TitleTip.Show(TextRect, pCell->GetDesText(), 0, CellRect,
...
void CGridCtrl::ReleaseMergeCells()
{
...
{ 	
	...
	//////////////////////////////////////////////////////////////////////////
	//20210513 JBP FiexdRow 수정 MergeCell이 안되서 주석
...
// 20210828 by jyoon : Merge 된 Range를 가져옴
BOOL CGridCtrl::GetMergedCellRange(int row, int col, CCellRange& cellRange)
{
	INT_PTR size = m_arMergedCells.GetSize();
	if (size > 0)
	{
		for (INT_PTR i = 0; i < size; i++)
		{
			if (IsMergedCell(row, col, m_arMergedCells[i]))
			{
				...
				cellRange.SetMaxCol(nMaxCol);
				cellRange.SetMinCol(nMinCol);
