...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            */
            // Added by Yogurt
#if 0	/* Modified by JsKang 2017.05.04 */
...
#endif
			CPoint ptImg	= rect.TopLeft();
			if ((DT_CENTER == (GetFormat() & DT_CENTER)) || (DT_VCENTER == (GetFormat() & DT_VCENTER)))
			{
				if (DT_CENTER == (GetFormat() & DT_CENTER))
				{
					ptImg.x	= rect.left + (rect.Width() / 2 - (Info.rcImage.right - Info.rcImage.left) / 2);
				}
				if (DT_VCENTER == (GetFormat() & DT_VCENTER))
				{
					ptImg.y	= rect.top + (rect.Height() / 2 - (Info.rcImage.bottom - Info.rcImage.top) / 2);
				}
				pGrid->GetImageList()->Draw(pDC, GetImage(), ptImg, ILD_NORMAL);
			}
			else
			{
				pGrid->GetImageList()->Draw(pDC, GetImage(), rect.TopLeft(), ILD_NORMAL);
