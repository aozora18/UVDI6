...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
...
#else
	if (!pState->m_bNeedTerm && !CoInitializeEx(NULL, COINIT_MULTITHREADED||COINIT_DISABLE_OLE1DDE))
#endif
    ...
    m_crShadow              = RGB(255, 255, 255);
    m_crGridLineColour      = RGB(112, 128, 144);
    ...
    m_bUseNotifyEvent       = FALSE;      // 230418 mhbaek Add
	...
	m_uID				 	= 0;
	m_pParent			 	= NULL;
	m_bDrawScrollBarHorz 	= TRUE;		// 기본적으로 수평 스크롤바 그려줌
	m_bDrawScrollBarVert 	= TRUE;		// 기본적으로 수직 스크롤바 그려줌
	m_ptXY.x			 	= 0;
	m_ptXY.y			 	= 0;
	m_bEnableClickDown		= TRUE;
	m_bEnableClickUp		= TRUE;
	m_bEnableDblClick		= TRUE;
	m_bSelfSendMessage		= FALSE;	// 일단 자신을 호출한 부모에게 SendMessage 전송하도록 설정
	m_bFocuseCell			= FALSE;
    // by sysandj : UI 깔끔하게
	//m_u32StyleEx			= WS_EX_CLIENTEDGE;
    m_u32StyleEx			= WS_BORDER;
    // by sysandj : UI 깔끔하게
...
//#endif
	/* Modified by JsKang 20201204 */
    if (::IsWindow(m_hWnd))	ModifyStyleEx(0, m_u32StyleEx);
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage)
{
	...
	// Modified by JsKang 2015-10-13
	if (!m_bEnableClickDown)	return 0L;
	...
	else
	{
		...
		case LVN_FIRST	:
#endif
		case NM_CLICK	:	CellLButtonUp(nRow, nCol);	break;
		}
#endif
	}
#endif
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if (next != m_idCurrentCell)
    {
        ...
        if (m_bFocuseCell)	SetFocusCell(next);
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    *pResult = 0;
	...
	// Modified by JsKang ...포커스를 잃은 경우, 부모에게 알려준다.
	if(m_pParent)	m_pParent->SendMessage(WM_GRID_WINDOW_KILL_FOCUS, 0, 0L);	// Modified by JsKang
}
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)
    {
        ...
        for (col = minVisibleCol; col <= maxVisibleCol; col++)
        {
            ...
            // by sysandj : guide line 추가
            if (col == 0)
            {
                pDC->MoveTo(x, nFixedRowHeight);
...
void CGridCtrl::ResetSelectedRange()
{
    ...
    if (m_bFocuseCell)	SetFocusCell(-1,-1);
}
...
void CGridCtrl::EnableScrollBars(int nBar, BOOL bEnable /*=TRUE*/)
{
    if (bEnable)
    {
        if (m_bDrawScrollBarHorz && !IsVisibleHScroll() && (nBar == SB_HORZ || nBar == SB_BOTH))
        {
            m_nBarState |= GVL_HORZ;
            CWnd::EnableScrollBarCtrl(SB_HORZ, bEnable);
        }
        
        if (m_bDrawScrollBarVert && !IsVisibleVScroll() && (nBar == SB_VERT || nBar == SB_BOTH))
        {
...
int CGridCtrl::InsertRow(int value, int nRow /* = -1 */)
{
#ifdef	_UNICODE
	TCHAR strHeading[1024]	= {NULL};
	swprintf_s(strHeading, 1024, L"%d", value);
#else
	char strHeading[1024]	= {NULL};
	sprintf_s(strHeading, 1024, "%d", value);
#endif

	return InsertRow(strHeading, nRow);
}
int CGridCtrl::InsertRow(UINT value, int nRow /* = -1 */)
{
#ifdef	_UNICODE
	TCHAR strHeading[1024]	= {NULL};
	swprintf_s(strHeading, 1024, L"%d", value);
...
int CGridCtrl::InsertRow(DOUBLE value, int nRow /* = -1 */)
{
#ifdef	_UNICODE
	TCHAR strHeading[1024]	= {NULL};
	swprintf_s(strHeading, 1024, L"%.4f", value);
#else
	char strHeading[1024]	= {NULL};
	sprintf_s(strHeading, 1024, "%.4f", value);
#endif
...
BOOL CGridCtrl::SortTextItemsUpper(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
    return CGridCtrl::SortItems(pfnCellTextCompareUpper, nCol, bAscending, data);
}

BOOL CGridCtrl::SortTextItemsLower(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
    return CGridCtrl::SortItems(pfnCellTextCompareLower, nCol, bAscending, data);
}
...
int CALLBACK CGridCtrl::pfnCellTextCompareUpper(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);

	CGridCell* pCell1 = (CGridCell*) lParam1;
	CGridCell* pCell2 = (CGridCell*) lParam2;
	if (!pCell1 || !pCell2) return 0;

	return _tcscmp(pCell1->GetTextStr().MakeUpper(), pCell2->GetTextStr().MakeUpper());
}

int CALLBACK CGridCtrl::pfnCellTextCompareLower(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);
	...
	return _tcscmp(pCell1->GetTextStr().MakeLower(), pCell2->GetTextStr().MakeLower());
}
...
BOOL CGridCtrl::SetItemText(int nRow, int nCol, DOUBLE value, BOOL space)
{
#ifdef	_UNICODE
	TCHAR str[1024]	= {NULL};
	if (!space)	swprintf_s(str, 1024, L"%.4f", value);
	else		swprintf_s(str, 1024, L"%.4f ", value);
#else
	char str[1024]	= {NULL};
	if (!space)	sprintf_s(str, 1024, "%.4f", value);
	else		sprintf_s(str, 1024, "%.4f ", value);
#endif

	return SetItemText(nRow, nCol, str);
}

BOOL CGridCtrl::SetItemFloat(int nRow, int nCol, FLOAT value, UINT8 point, BOOL space)
{
#ifdef	_UNICODE
	TCHAR str[1024]		= {NULL};
	TCHAR szPoint[32]	= {NULL};
	if (!space)	swprintf_s(szPoint, _T("%%.%df"), point);
	else		swprintf_s(szPoint, _T("%%.%df "), point);
	swprintf_s(str, 1024, szPoint, value);
#else
	char str[1024]		= {NULL};
	char szPoint[32]	= {NULL};
	swprintf_s(szPoint, "%%.%df", point);
	sprintf_s(str, 1024, "%d", data);
#endif

	return SetItemText(nRow, nCol, str);
}

BOOL CGridCtrl::SetItemDouble(int nRow, int nCol, DOUBLE value, UINT8 point)
{
#ifdef	_UNICODE
	TCHAR str[1024]	= {NULL};
	TCHAR szPoint[32]	= {NULL};
...
BOOL CGridCtrl::SetItemHex32(int nRow, int nCol, int value, BOOL space)
{
#ifdef	_UNICODE
	TCHAR str[1024]	= {NULL};
	if (!space)	swprintf_s(str, 1024, L"%X", value);
	else		swprintf_s(str, 1024, L"%X ", value);
#else
	char str[1024]	= {NULL};
	if (!space)	sprintf_s(str, 1024, "%X", value);
	else		sprintf_s(str, 1024, "%X ", value);
#endif

	return SetItemText(nRow, nCol, str);
}

BOOL CGridCtrl::SetItemText(int nRow, int nCol, int value, BOOL space)
{
#ifdef	_UNICODE
	TCHAR str[1024]	= {NULL};
	if (!space)	swprintf_s(str, 1024, L"%d", value);
	else		swprintf_s(str, 1024, L"%d ", value);
#else
	char str[1024]	= {NULL};
	if (!space)	sprintf_s(str, 1024, "%d", value);
	else		sprintf_s(str, 1024, "%d ", value);
#endif

	return SetItemText(nRow, nCol, str);
}
BOOL CGridCtrl::SetItemText(int nRow, int nCol, UINT value, BOOL space)
{
#ifdef	_UNICODE
	TCHAR str[1024]	= {NULL};
	if (!space)	swprintf_s(str, 1024, L"%d", value);
...
//						TRACE0("Showing TitleTip\n");
        ...
        if (NULL != m_pParent)
        {
            m_pParent->PostMessage(WM_GRID_MOUSE_MOVE, 0, (LPARAM)&m_LastMousePoint);	// Modified by JsKang 2009.09.17
        }
        ...
        case MOUSE_SELECT_CELLS:
            {
                ...
                if (idCurrentCell != GetFocusCell())
                {
                    ...
                    if((idCurrentCell.row >= m_nFixedRows &&
                      ...
                      m_MouseMode==MOUSE_SELECT_ROW)
                    {
                        if (m_bFocuseCell)	SetFocusCell(idCurrentCell);
                    }
                }
                break;
            }
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_NOTHING)
    {
            ...
            cell.col >= m_nFixedCols && bInTextArea)
        {
			...
			// Modified by JsKang 20030108
			if(m_pParent)
			{
				// Modified by JsKang 20140528
				m_ptXY.x	= cell.row;
				m_ptXY.y	= cell.col;
			}
		}
		...
		// Modified by JsKang 2015.10.13
		if (m_bEnableDblClick)	CellLButtonDblDown(cell.row, cell.col);

    }

    CWnd *pOwner = GetOwner();
    if (pOwner && IsWindow(pOwner->m_hWnd))
		pOwner->SendMessage(WM_COMMAND, MAKELONG(GetDlgCtrlID(), BN_CLICKED), (LPARAM) GetSafeHwnd());
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    {
        if (m_bFocuseCell)	SetFocusCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);

        // If control is pressed then unselect the cell or row (depending on the list mode)
        if (nFlags & MK_CONTROL)
        {
            if (m_bFocuseCell)	SetFocusCell(m_LeftClickDownCell);
            if (GetListMode())
    ...
    if (m_MouseMode == MOUSE_NOTHING)
    {
        ...
        //}
		// Modified by JsKang 2015.10.13
		if (m_bEnableClickDown)
		{
			CellLButtonDown(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
			// Modified by JsKang 20030108
			if(m_pParent)
			{
				// Modified by JsKang 20140528
				m_ptXY.x	= m_LeftClickDownCell.row;
				m_ptXY.y	= m_LeftClickDownCell.col;
			}
		}
     }
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_PREPARE_DRAG) 
    {
        ...
        // 230418 mhbaek Modify : Use Notify Event
        if (TRUE == m_bUseNotifyEvent)
        {
    ...
    else
    {
		...
		// Modified by JsKang 2015.10.13
		if (m_bEnableClickUp)	CellLButtonUp(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    ...
    {
        if (m_bFocuseCell)	SetFocusCell(-1,-1);
        if (m_bFocuseCell)	SetFocusCell(__max(FocusCell.row, m_nFixedRows), __max(FocusCell.col, m_nFixedCols));
...
void CGridCtrl::OnFixedRowClick(CCellID& cell)
{
    ...
    if (GetHeaderSort())
    {
        CWaitCursor waiter;
#if 1
        if (cell.col == GetSortColumn())	SortItems(cell.col, !GetSortAscending());
        else								SortItems(cell.col, TRUE);
#else
        if (cell.col == GetSortColumn())	SortTextItemsUpper(cell.col, !GetSortAscending());
        else								SortTextItemsUpper(cell.col, TRUE);
#endif
        Invalidate();
    }
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
    ...
    if (strCurrentText != str)
    {
            ...
            SendMessageToParent(nRow, nCol, GVN_ENDLABELEDIT) >= 0)
        {
            SetModified(TRUE, nRow, nCol);
            RedrawCell(nRow, nCol);
			// 에디트 컨트롤 내에 입력이 완료 된 경우
			m_pParent->PostMessage(WM_GRID_EDIT_STRING_INPUT_COMPLETED, 0, (LPARAM)this);	// Modified by JsKang 2004.06
        }
        else
        {
            SetItemText(nRow, nCol, strCurrentText);
...
int CGridCtrl::GetItemTextToInt(int nRow, int nCol) const
{
	if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols)
		return 0;
	...
	return _wtoi(pCell->GetText());
}

DOUBLE CGridCtrl::GetItemTextToFloat(int nRow, int nCol) const
{
	if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols)
		return 0.0f;
...
/*
 desc : 그리드 컨트롤 초기화 작업
*/
void CGridCtrl::SetBaseGridProp(int fixed_row_cnt, int fixed_col_cnt,
								BOOL tooltip, BOOL list_mode, BOOL drag_drop, BOOL is_edit,
								BOOL resize_row, BOOL resize_col, BOOL sort, BOOL single_select,
								BOOL focusecell)
{
	SetFixedRowCount(fixed_row_cnt);
	SetFixedColumnCount(fixed_col_cnt);
	SetListMode(list_mode);
	SetEditable(is_edit);
	SetRowResize(resize_row);
	SetColumnResize(resize_col);
	EnableDragAndDrop(drag_drop);
	EnableTitleTips(tooltip);
	SetHeaderSort(sort);
	SetSingleRowSelection(single_select);
    SetSingleColSelection(!single_select);
	EnableSelection(TRUE);
	m_bFocuseCell	= focusecell;
}

void CGridCtrl::UpdateGrid()
{
	Invalidate();
	ResetScrollBars();
}

void CGridCtrl::SetItemInfo(int nRow, int nCol, LOGFONT *lf, UINT nFormat)
{
	SetItemFont(nRow, nCol, lf);
	SetItemFormat(nRow, nCol, nFormat);
}

/*
 desc : 각 행의 모든 칼럼에 대해서 갱신
 parm : row	- 행의 위치 (0 기반~)
 retn : None
*/
void CGridCtrl::InvalidateRow(int row)
{
	...
	// 해당 행의 모든 컬럼에 대해서 갱신
	for (i=0; i<iCol; i++)	InvalidateCellRect(row, i);
}
...
/*
 ...
 retn : None
*/
void CGridCtrl::InvalidateRowCol(int row, int col)
{
	// 해당 행의 모든 컬럼에 대해서 갱신
	InvalidateCellRect(row, col);
}
...
/*
 desc : 그리드 컨트롤 내의 임의 행과 컬럼에 대한 속성 설정
 parm : row		- [in]  그리드 컨트롤의 행 위치
		col		- [in]  그리드 컨트롤의 열 위치
		align	- [in]  컬럼 내의 문자열 위치 (왼쪽:0, 가운데:1, 오른쪽:2)
		height	- [in]  각 행의 높이 값
 retn : None
*/
VOID CGridCtrl::SetGridFormat(UINT32 row, UINT32 col, UINT8 align, UINT32 height)
{
	UINT32 uiFormat	= DT_VCENTER|DT_SINGLELINE;

	switch (align)
	{
	case 0x00	:	uiFormat |= DT_LEFT;	break;
	case 0x01	:	uiFormat |= DT_CENTER;	break;
	case 0x02	:	uiFormat |= DT_RIGHT;	break;
	}

	// 각 행의 높이 값 설정
	if (height)	SetRowHeight(row, height);
	// 포맷 설정
	SetItemInfo((INT32)row, (INT32)col, &m_Logfont, uiFormat);
	// 해당 행과 열 갱신
	InvalidateCellRect(row, col);
}

/*
 desc : 고정 칼럼 영역이 아닌 데이터 영역 모두 지우기
 parm : None
 retn : None
*/
VOID CGridCtrl::ResetNonItems()
{
	UINT32 i, j;
	UINT32 u32Rows	= GetRowCount();
	UINT32 u32Cols	= GetColumnCount();
	UINT32 u32RowH	= GetFixedRowCount();
	UINT32 u32ColH	= GetFixedColumnCount();

	for (i=u32RowH; i<u32Rows; i++)
	{
		for (j=u32ColH; j<u32Cols; j++)
		{
			SetItemText(i, j, L"");
		}
	}
...
/*
 desc : 셀에 들어 있는 값과 문자열 비교
 parm : row	- [in]  비교하려는 문자열이 등록된 행
		col	- [in]  비교하려는 문자열이 등록된 열
		str	- [in]  비교하려는 문자열이 저장된 버퍼 포인터
 retn : TRUE (동일하다) or FALSE (다르다)
*/
BOOL CGridCtrl::IsEqualText(INT32 row, INT32 col, TCHAR *str)
{
	return 0 == GetItemText(row, col).Compare(str) ? TRUE : FALSE;
}

/*
 desc : 현재 보여지고 있는 화면에서 Row 시작과 끝 위치 값 (Index; Row Item) 반환
 parm : begin	- [out] Row Index (Zero based)
		end		- [out] Row Index (Zero based) 단, begin보다 작으면 안됨
						그리고, 최대 값이 전체 Row 개수보다 1이 항상 작아야 함
 retn : TRUE or FALSE
*/
BOOL CGridCtrl::GetViewRowIndex(UINT32 &begin, UINT32 &end)
{
	INT32 i, i32RowHeight = 0;
	CRect rClient;
	CCellID csCellTopLeft	= GetTopleftNonFixedCell();	/* 현재 위치의 Top Row Index 구하기 */

	/* 현재 등록된 행의 개수가 없는지 확인 */
    ...
    if (IsVisibleHScroll())
	{
		rClient.bottom += GetSystemMetrics(SM_CYHSCROLL) + GetSystemMetrics(SM_CYBORDER);
	}
    rClient.left	+= GetFixedColumnWidth();
    rClient.top		+= GetFixedRowHeight();

	/* 현재 Begin Row Index 값 설정 */
	begin	= csCellTopLeft.row;

	/* 그리드 영역 크기만큼 행의 개수 구하기 */
	for (i=0, end=begin; i<GetRowCount(); i++, end++)
	{
		i32RowHeight	+= GetRowHeight(i);
		if (i32RowHeight > rClient.Height())	break;
	}
	/* End Row Index 값이 RowCount 값보다 크면 안됨 */
	if (end >= UINT32(m_nRows))	end	= UINT32(m_nRows - 1);
