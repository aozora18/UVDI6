...
#define	WM_GRID_EDIT_STRING_INPUT_COMPLETED		WM_USER+32766	// Modified by jskang21 2004.06
#define	WM_GRID_WINDOW_KILL_FOCUS				WM_USER+32765	// Modified by jskang21 2004.12
#define	WM_GRID_MOUSE_MOVE						WM_USER+32762	// Modified by jskang21 2009.09
...
public:
	...
	// Modified by JsKang 20090302
	void					SetBaseGridProp(int fixed_row_cnt=1, int fixed_col_cnt=0,
									BOOL tooltip=TRUE, BOOL list_mode=TRUE, BOOL drag_drop=FALSE,
									BOOL is_edit=FALSE, BOOL resize_row=FALSE, BOOL resize_col=FALSE,
									BOOL sort=FALSE, BOOL single_select=FALSE, BOOL focusecell=TRUE);
	// Modified by JsKang 20150521
	VOID SetGridFormat(UINT32 row, UINT32 col, UINT8 align, UINT32 height=0);
	// Modified by JsKang 20150521
	VOID ResetNonItems();
	// Modified by JsKang 20090422
	void					UpdateGrid();
	// Modified by JsKang 20090917
	CGridCell				*GetGridCel(int nRow, int nCol)	{	return (CGridCell*)GetCell(nRow, nCol);	}
	// Modified by JsKang 2011-04-22
	void SetItemInfo(int nRow, int nCol, LOGFONT *lf, UINT nFormat);
	// Modified by JsKang 2012-01-16
	void InvalidateRow(int row);
	void InvalidateRowCol(int row, int col);
	/* Modified by JsKang 20201204 */
	VOID SetModifyStyleEx(UINT32 style)		{	m_u32StyleEx			= style;};

	// Modified or Appended by JsKang
	void SetDrawScrollBarHorz(BOOL flag)	{	m_bDrawScrollBarHorz	= flag; }
	void SetDrawScrollBarVert(BOOL flag)	{	m_bDrawScrollBarVert	= flag; }
	// Modified or Appended by JsKang 20030108
	POINT *GetSelPos()				{	return &m_ptXY;		}
	VOID ResetSelPos()				{	m_ptXY.x = m_ptXY.y = -1;	}
	// Modified or Appended by JsKang 20090630
	LOGFONT *GetLogFont()	{	return &m_Logfont;	}
	void SetLogFont(LOGFONT lf)	{	memcpy(&m_Logfont, &lf, sizeof(LOGFONT));	}
	// Modified or Appended by JsKang 20150820
	void SetEnableDblClick(BOOL enable)		{	m_bEnableDblClick	= enable;	}
	void SetEnableClickDown(BOOL enable)	{	m_bEnableClickDown	= enable;	}
	void SetEnableClickUp(BOOL enable)		{	m_bEnableClickUp	= enable;	}
	BOOL IsEqualText(INT32 row, INT32 col, TCHAR *str);
	BOOL GetViewRowIndex(UINT32 &begin, UINT32 &end);	/* Appended by JsKang 20200908 */

	void SetUseNotifyEvent(BOOL bUse) { m_bUseNotifyEvent = bUse; }
	BOOL GetUseNotifyEvent() const { return m_bUseNotifyEvent; }
...
public:
	...
	// Modified by JsKang 2015.08.26
	BOOL   SetItemText(int nRow, int nCol, DOUBLE value, BOOL space=FALSE);
	BOOL   SetItemFloat(int nRow, int nCol, FLOAT value, UINT8 point, BOOL space=FALSE);
	BOOL   SetItemDouble(int nRow, int nCol, DOUBLE value, UINT8 point=0);
	BOOL   SetItemHex32(int nRow, int nCol, int value, BOOL space=FALSE);
	BOOL   SetItemText(int nRow, int nCol, UINT value, BOOL space=FALSE);
	BOOL   SetItemText(int nRow, int nCol, int value, BOOL space=FALSE);
	...
	int		GetItemTextToInt(int nRow, int nCol) const;
	DOUBLE	GetItemTextToFloat(int nRow, int nCol) const;
...
public:
    int  InsertColumn(LPCTSTR strHeading, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,
                      int nColumn = -1);
	int  InsertRow(int value, int nRow = -1);
	int  InsertRow(UINT value, int nRow = -1);
	int  InsertRow(DOUBLE value, int nRow = -1);
...
public:
    ...
    BOOL SortTextItemsUpper(int nCol, BOOL bAscending, LPARAM data = 0);
    BOOL SortTextItemsLower(int nCol, BOOL bAscending, LPARAM data = 0);
	...
	static int CALLBACK pfnCellTextCompareUpper(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
	static int CALLBACK pfnCellTextCompareLower(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
...
protected:
	...
	// Modified by JsKang 2015.10.13 (실제로 아무것도 하지 않는다.)
	virtual VOID CellLButtonDown(INT32 row, INT32 col)		{};
	virtual VOID CellLButtonUp(INT32 row, INT32 col)		{};
	virtual VOID CellLButtonDblDown(INT32 row, INT32 col)	{};
...
protected:
	...
	// Modified by JsKang
	BOOL		m_bSelfSendMessage;	// 자기 자신에게 메시지를 보낼지 말지
	BOOL		m_bEnableDblClick;
	BOOL		m_bEnableClickDown;
	BOOL		m_bEnableClickUp;
	BOOL		m_bDrawScrollBarHorz;
	BOOL		m_bDrawScrollBarVert;
	BOOL		m_bFocuseCell;
	UINT		m_uID;
	CWnd		*m_pParent;
	POINT		m_ptXY;
    ...
    // 230418 mhbaek Add : Notify Event
    BOOL        m_bUseNotifyEvent;
