...
/*
 ...
 retn : 등록된 개수 반환
*/
API_EXPORT UINT32 uvEng_Recipe_GetCount()
{
...
*/
API_EXPORT LPG_RBGF uvEng_Recipe_GetSelectRecipe()
{
...
/*
 ...
 retn : TRUE or FALSE
*/
API_EXPORT BOOL uvEng_Recipe_CalcSpeedEnergy(LPG_RBGF recipe)
{
...
{
	CUniToChar csCnv;
	return g_pRecipe->RecipeFind(csCnv.Uni2Ansi(recipe));
}
...
*/
API_EXPORT LPG_RBGF uvEng_Recipe_GetRecipeOnlyName(PTCHAR recipe)
{
	CUniToChar csCnv;
	return g_pRecipe->GetRecipeOnlyName(csCnv.Uni2Ansi(recipe));
}
...
/*
 ...
 retn : 반환될 레시피가 저장된 구조체 포인터
*/
API_EXPORT LPG_RBGF uvEng_Recipe_GetRecipePathName(PTCHAR name)
{
	CUniToChar csCnv;
	return g_pRecipe->GetRecipePathName(csCnv.Uni2Ansi(name));
}
...
/*
 ...
 retn : 반환될 레시피가 저장된 구조체 포인터
*/
API_EXPORT LPG_RBGF uvEng_Recipe_GetRecipeIndex(INT32 index)
{
	return g_pRecipe->GetRecipeIndex(index);
}

API_EXPORT int uvEng_Recipe_GetSelectRecipeIndex()
{
	return g_pRecipe->GetSelectRecipeIndex();
}
...
*/
API_EXPORT BOOL uvEng_Recipe_SelRecipeOnlyName(PTCHAR recipe)
{
	CUniToChar csCnv;
	return g_pRecipe->SelRecipeOnlyName(csCnv.Uni2Ansi(recipe));
}
...
/*
 ...
 retn : TRUE (선택 성공) or FALSE (선택 실패)
*/
API_EXPORT BOOL uvEng_Recipe_SelRecipePathName(PTCHAR recipe)
{
	CUniToChar csCnv;
	return g_pRecipe->SelRecipePathName(csCnv.Uni2Ansi(recipe));
}
...
/*
 ...
 retn : TRUE or FALSE
*/
API_EXPORT BOOL uvEng_Recipe_SaveFile()
{
	return g_pRecipe->SaveFile();
}
...
*/
API_EXPORT BOOL uvEng_Recipe_RecipeAppend(LPG_RBGF recipe)
{
...
/*
 ...
 retn : TRUE or FALSE
*/
API_EXPORT BOOL uvEng_Recipe_RecipeModify(LPG_RBGF recipe)
{
...
{
	CUniToChar csCnv;
	return g_pRecipe->RecipeDelete(csCnv.Uni2Ansi(recipe));
}
