...
{
	TCHAR tzIPv4[LEN_IPv4]	= {NULL};

	GetPrivateProfileString(sub, key, L"", tzIPv4, LEN_IPv4, m_tzFile);
	/* 읽어들인 IPv4 정보가 없다면... */
...
{
	TCHAR tzIPv4[LEN_IPv4]	= {NULL};
	uvCmn_GetIPv4Uint32ToStr(ipv4, tzIPv4, LEN_IPv4);
...
{
	TCHAR tzIPv4[LEN_IPv4]	= {NULL};
	swprintf_s(tzIPv4, LEN_IPv4, L"%d.%d.%d.%d", ipv4[0], ipv4[1], ipv4[2], ipv4[3]);
	SetConfigStr(sub, key, tzIPv4);
}
