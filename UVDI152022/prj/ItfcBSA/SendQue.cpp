...
CSendQue::CSendQue(UINT32 count)
{
	...
	m_pstQue = new STG_PBSR[count];
	ASSERT(m_pstQue);
	memset(m_pstQue, 0x00, sizeof(STG_PBSR)*count);
}
...
BOOL CSendQue::PushData(UINT16 cmd, PUINT8 data, UINT16 size)
{
	...
	if (IsQueFull())
	{
		LOG_ERROR(ENG_EDIC::en_bsa, L"There is not enough space in the queue. It is full.");
		return FALSE;
	}

	/* Header */
	stHead.head		= (UINT16)ENG_PHSI::en_head_vision_vs_cmps;
	stHead.cmd		= cmd;
	stHead.length	= u16PktSize = sizeof(STG_UTPH) + size + sizeof(STG_UTPT);
	/* Tailer */
	stTail.crc		= 0x00;
	stTail.tail		= (UINT16)ENG_PHSI::en_comn_tail;
	/* Big Endian */
