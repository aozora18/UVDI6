...
const double REAL_MAX       = DBL_MAX/*numeric_limits<double>::max()*/;
const double REAL_MIN       = DBL_MIN/*numeric_limits<double>::min()*/;
const double REAL_EPSILON   = DBL_EPSILON/*numeric_limits<double>::epsilon()*/;
...
/*
 desc : Constructor with assignment of the circle parameters only
 parm : aa  - [in]  Center X
        bb  - [in]  Center Y
        rr  - [in]  Radius (Half diameter)
 retn : None
*/
CCircle::CCircle(double aa, double bb, double rr)
{
    a=aa; b=bb; r=rr;
}
...
CCircleData::CCircleData(int N, double dataX[], double dataY[])
{
    n   = N;
    X   = new double[n];
    Y   = new double[n];
...
double CCircleFit::Sigma(CCircleData* data, CCircle& circle)
{
    double sum=0.,dx,dy;
...
/*
 ...
 retn : TRUE or FALSE
*/
BOOL CCircleFit::GetFit(UINT32 count, DOUBLE *coord_x, DOUBLE *coord_y, UINT8 method,
                        DOUBLE &cent_x, DOUBLE &cent_y)
{
    if (method > 0x03)  return FALSE;

    CCircleData *pData  = new CCircleData(count, coord_x, coord_y);
    ASSERT(pData);
    ...
    switch (method)
    {
    case 0x00 : csCircle = CircleFitByHyper(pData);     break;
    case 0x01 : csCircle = CircleFitByKasa(pData);      break;
    case 0x02 : csCircle = CircleFitByPratt(pData);     break;
    case 0x03 : csCircle = CircleFitByTaubin(pData);    break;
    }
    delete pData;

    /* Return the center xy of coordination */
    cent_x  = csCircle.a;
    cent_y  = csCircle.b;
...
/*
 desc : Circle fit to a given set of data points (in 2D)
 parm : None
 retn : TRUE or FALSE
 note : This is an algebraic fit based on the journal article
...
*/
CCircle CCircleFit::CircleFitByHyper(CCircleData *data)
{
...
/*
 ...
 parm : data    - [in]  
 retn : Circle
 note : This is an algebraic fit, disovered and rediscovered by many people.
        ...
        Nikolai Chernov  (September 2012)
*/
CCircle CCircleFit::CircleFitByKasa(CCircleData *data)
{
    int i;
...
/*
 ...
 parm : 
 retn : CCircle
 note : This is an algebraic fit, due to Pratt, based on the journal article
        ...
        Nikolai Chernov  (September 2012)
*/
CCircle CCircleFit::CircleFitByPratt(CCircleData *data)
{
    int i,iter,IterMAX=99;
...
/*
 ...
 parm : 
 retn : CCircle
 note : This is an algebraic fit, due to Taubin, based on the journal article
        ...
        Nikolai Chernov  (September 2012)
*/
CCircle CCircleFit::CircleFitByTaubin(CCircleData *data)
{
    int i,iter,IterMAX=99;
...
/*
 ...
 parm : 
 retn : CCircle
 note : Input:  data     - the class of data (contains the given points):
                data->n   - the number of data points
                data->X[] - the array of X-coordinates
                data->Y[] - the array of Y-coordinates
...
*/
int CCircleFit::CircleFitByLevenbergMarquardtFull(CCircleData *data, CCircle &circleIni, double LambdaIni, CCircle &circle)
{
