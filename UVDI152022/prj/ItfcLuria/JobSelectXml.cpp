							  ...
							  CAtlList <STG_XMXY> &mark_xy)
{
	...
	else
	{
		...
		else
		{
			...
			i32Next		= row * 2;		/* 1 Strip 마다 상/하 2개씩 Fiducial이 있고, row 개수만큼 있으므로 */
			i32Loop		= col * 2;
		}
	}
...
BOOL CJobSelectXml::LoadRegistrationXML(CHAR *job_name, ENG_ATGL align_type)
{
		...
		align_type != ENG_ATGL::en_global_2_local_0x0_n_point)
	{
		...
		if (xmlElem2)
		{
			...
			bSharedType	= IsSharedAlignType(align_type);
			if (bSharedType)
			{
				u16Col	= 3;	/* Fixed */
				u16Cnt	= 0;	/* Fixed */
			...
			// by sysandj : 주석
			//switch (align_type)
			BOOL bNotDefined = FALSE;
			// by sysandj : 현재 마크를 자동으로 가져와 계산되도록 변경
			// abh1000 Local
			//int nAllMarkCount = m_pFidGlobal->GetCount() + lstGbrXY.GetCount() - 1;
			//switch (nAllMarkCount)
			switch (align_type)
			{
			/* normal */
			case ENG_ATGL::en_global_4_local_2x1_n_point : u16Col = 1;			/* Global (4) points / Local Division (1 x 2) (16) points */
			case ENG_ATGL::en_global_4_local_2x2_n_point : u16Row = 2;	break;	/* Global (4) points / Local Division (2 x 2) (16) points */
			case ENG_ATGL::en_global_4_local_3x2_n_point : u16Row = 3;	break;	/* Global (4) points / Local Division (3 x 2) (24) points */
			case ENG_ATGL::en_global_4_local_4x2_n_point : u16Row = 4;	break;	/* Global (4) points / Local Division (4 x 2) (32) points */
			case ENG_ATGL::en_global_4_local_5x2_n_point : u16Row = 5;	break;	/* Global (4) points / Local Division (5 x 2) (40) points */
			/* shared */								 
			case ENG_ATGL::en_global_4_local_2x2_s_point : u16Row = 3;	break;	/* Global (4) points / Local Division (2 x 2) (13) points */
			case ENG_ATGL::en_global_4_local_3x2_s_point : u16Row = 4;	break;	/* Global (4) points / Local Division (3 x 2) (16) points */
			case ENG_ATGL::en_global_4_local_4x2_s_point : u16Row = 5;	break;	/* Global (4) points / Local Division (4 x 2) (19) points */
			case ENG_ATGL::en_global_4_local_5x2_s_point : u16Row = 6;	break;	/* Global (4) points / Local Division (5 x 2) (22) points */
			default										 : bNotDefined = TRUE; break;
			}
											...
											CAtlList <DOUBLE> &cam1, CAtlList <DOUBLE> &cam2)
{
	...
	for (i=u8Count; i>0; i--)
	{
		...
		if (!m_pFidLocal->GetMark(u8CamX1+i-1, stTgt[0]) ||
			!m_pFidLocal->GetMark(u8CamX2+i-1, stTgt[1]))
		{
			cam1.RemoveAll();
			cam2.RemoveAll();
			LOG_ERROR(ENG_EDIC::en_luria, L"Failed to get the mark of Left or Bottom");
