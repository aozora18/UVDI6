...
CPhotoheadFocus::CPhotoheadFocus(LPG_LDSM shmem)
: CBaseFamily(shmem, nullptr)
{
	m_enFamily = ENG_LUDF::en_focus;
	m_pstShMem = &shmem->focus;
}

unique_ptr<UINT8[]> GetSendingBuffer()
{
	auto ptr = make_unique<UINT8[]>(128);
	ZeroMemory(ptr.get(), 128);
	return ptr;
}

void CopyBuffer(PUINT8 destPtr, void* srcPtr, int length, int& writePos)
{
	memcpy(destPtr + writePos, &srcPtr, length);
...
/// </summary>
/// <returns></returns>
unique_ptr<UINT8[]> CPhotoheadFocus::SendPktInitializeFocus() //write only
{
	auto sendingBuff = GetSendingBuffer();
	
	auto pPktNext = GetPktBase(ENG_LTCT::en_write, ENG_LCPF::en_initialize_focus,
					sendingBuff.get(), 128, UINT8{0}, 0);
...
/*
 ...
 retn : None
*/
VOID CPhotoheadFocus::SetRecvPacket(UINT8 uid, PUINT8 data, UINT32 size)
{
	switch ((ENG_LCPF)uid)
	{
		...
		break;
		case ENG_LCPF::en_position:
		{

		}
		break;
		case ENG_LCPF::en_auto_focus:
		{

		}
		break;
		case ENG_LCPF::en_trim:
		{

		}
		break;
		case ENG_LCPF::en_active_area_qualifier:
		{

		}
		break;
		case ENG_LCPF::en_abs_work_range:
		{

		}
		break;
		case ENG_LCPF::en_abs_work_range_status:
		{

		}
		break;
		case ENG_LCPF::en_af_edge_trigger:
		{

		}
		break;
		case ENG_LCPF::en_set_position_multi:
		{

		}
		break;
		case ENG_LCPF::en_position_hi_res:
		{

		}
		break;
		case ENG_LCPF::en_outside_dof_status:
		{

		}
		break;
		case ENG_LCPF::en_get_position_multi:
		{

		}
		break;
		case ENG_LCPF::en_af_gain:
		{

		}
		break;
		case ENG_LCPF::en_scan_ethercat:
		{

		}
		break;
		case ENG_LCPF::en_af_panelmode:
		{

		}
		break;
		case ENG_LCPF::en_af_disablefocus:
		{
...
/// <param name="zPosMicrometer"></param>
/// <returns></returns>
unique_ptr<UINT8[]> CPhotoheadFocus::SendPktFocusposition(ENG_LPGS flag,UINT8 headNum,INT32 zPosMicrometer) //Set or get
{
	auto sendingBuff = GetSendingBuffer();

	const int bodysize = 5;
	UINT8 body[bodysize] = { 0, }; int pos = 0;
	zPosMicrometer = SWAP32(zPosMicrometer);
	CopyBuffer(body, (void*)headNum, sizeof(UINT8), pos);
	CopyBuffer(body, (void*)zPosMicrometer, sizeof(INT32), pos);
	
	auto pPktNext = GetPktBase(ENG_LPGS::en_set == flag ? ENG_LTCT::en_write : ENG_LTCT::en_read, ENG_LCPF::en_position,
					sendingBuff.get(), 128, body, bodysize);
	
	return sendingBuff;
}

unique_ptr<UINT8[]> CPhotoheadFocus::SendPktActivateAutofocus(ENG_LPGS flag,UINT8 headNum, bool enable) //Start/Stop 
{
	auto sendingBuff = GetSendingBuffer();
	...
	UINT8 body[bodysize] = { 0, }; int pos = 0;
	CopyBuffer(body, (void*)headNum, sizeof(UINT8), pos);
	CopyBuffer(body, (void*)enable, sizeof(bool), pos);

	auto pPktNext = GetPktBase(ENG_LPGS::en_set == flag ? ENG_LTCT::en_write : ENG_LTCT::en_read, ENG_LCPF::en_auto_focus,
		sendingBuff.get(), 128, body, bodysize);
...
unique_ptr<UINT8[]> CPhotoheadFocus::SendPktFocustrim(ENG_LPGS flag,UINT8 headNum, INT32 trimNanometer)
{
	auto sendingBuff = GetSendingBuffer();
	trimNanometer = SWAP32(trimNanometer);
	const int bodysize = 5;
	UINT8 body[bodysize] = { 0, }; int pos = 0;
	CopyBuffer(body, (void*)headNum, sizeof(UINT8), pos);
	CopyBuffer(body, (void*)trimNanometer, sizeof(INT32), pos);

	auto pPktNext = GetPktBase(ENG_LPGS::en_set == flag ? ENG_LTCT::en_write : ENG_LTCT::en_read, ENG_LCPF::en_trim,
		sendingBuff.get(), 128, body, bodysize);
...
unique_ptr<UINT8[]> CPhotoheadFocus::SendPktActiveAreaQualifier(ENG_LPGS flag ,bool enable, UINT32 startInactiveInMicrometer, UINT32 EndInactiveInMicrometer)
{

	auto sendingBuff = GetSendingBuffer();

	const int bodysize = 9;
	UINT8 body[bodysize] = { 0, }; int pos = 0;
	startInactiveInMicrometer = SWAP32(startInactiveInMicrometer);
	EndInactiveInMicrometer = SWAP32(EndInactiveInMicrometer);
	CopyBuffer(body, (void*)enable, sizeof(bool), pos);
	CopyBuffer(body, (void*)startInactiveInMicrometer, sizeof(UINT32), pos);
	CopyBuffer(body, (void*)EndInactiveInMicrometer, sizeof(UINT32), pos);

	auto pPktNext = GetPktBase(ENG_LPGS::en_set == flag ? ENG_LTCT::en_write : ENG_LTCT::en_read, ENG_LCPF::en_active_area_qualifier,
		sendingBuff.get(), 128, body, bodysize);

	return sendingBuff;
}

unique_ptr<UINT8[]> CPhotoheadFocus::SendPktAbs_work_range(ENG_LPGS flag, UINT8 headNum, INT32 minFocusAFOperationMicrometer, INT32 maxFocusAFOperationMicrometer)
{
	...
	const int bodysize = 9;
	UINT8 body[bodysize] = { 0, }; int pos = 0;
	minFocusAFOperationMicrometer = SWAP32(minFocusAFOperationMicrometer);
	maxFocusAFOperationMicrometer = SWAP32(maxFocusAFOperationMicrometer);
	CopyBuffer(body, (void*)headNum, sizeof(UINT8), pos);
	CopyBuffer(body, (void*)minFocusAFOperationMicrometer, sizeof(INT32), pos);
	CopyBuffer(body, (void*)maxFocusAFOperationMicrometer, sizeof(INT32), pos);

	auto pPktNext = GetPktBase(ENG_LPGS::en_set == flag ? ENG_LTCT::en_write : ENG_LTCT::en_read, ENG_LCPF::en_abs_work_range,
		sendingBuff.get(), 128, body, bodysize);
...
unique_ptr<UINT8[]> CPhotoheadFocus::SendReadPktAbs_work_range_status()
{
	auto sendingBuff = GetSendingBuffer();
	auto pPktNext = GetPktBase(ENG_LTCT::en_read, ENG_LCPF::en_abs_work_range_status,sendingBuff.get(), 128);
	return sendingBuff;
}

//AF가 활성화될 현재 AF 설정점 아래 또는 위의 최대 거리. In 마이크로미터. 
unique_ptr<UINT8[]> CPhotoheadFocus::SendPktAf_edge_trigger(ENG_LPGS flag,UINT16 belowAbobeDistActivateAFinMicrometer , bool enable)
{
	auto sendingBuff = GetSendingBuffer();

	const int bodysize = 3;
	UINT8 body[bodysize] = { 0, }; int pos = 0;
	belowAbobeDistActivateAFinMicrometer = SWAP16(belowAbobeDistActivateAFinMicrometer);
	
	CopyBuffer(body, (void*)belowAbobeDistActivateAFinMicrometer, sizeof(UINT16), pos);
	CopyBuffer(body, (void*)enable, sizeof(bool), pos);
	
	auto pPktNext = GetPktBase(ENG_LPGS::en_set == flag ? ENG_LTCT::en_write : ENG_LTCT::en_read, ENG_LCPF::en_af_edge_trigger,
		sendingBuff.get(), 128, body, bodysize);
...
unique_ptr<UINT8[]> CPhotoheadFocus::SendPktDisableAF()
{
	auto sendingBuff = GetSendingBuffer();
	auto pPktNext = GetPktBase(ENG_LTCT::en_write, ENG_LCPF::en_af_disablefocus, sendingBuff.get(), 128);
	return sendingBuff;
}

unique_ptr<UINT8[]> CPhotoheadFocus::SendPktPanelMode(ENG_LPGS flag, UINT8 headNum, UINT8 mode)
{
	auto sendingBuff = GetSendingBuffer();
	...
	CopyBuffer(body, (void*)headNum, sizeof(UINT8), pos);
	CopyBuffer(body, (void*)mode, sizeof(UINT8), pos);
...
unique_ptr<UINT8[]> CPhotoheadFocus::SendPktPosition_multi(vector<tuple<UINT8,INT32>> positions)
{
	auto sendingBuff = GetSendingBuffer();
	UINT8 headCnt = (UINT8)positions.size();
	int bodysize = sizeof(UINT8) + ((sizeof(UINT8) + sizeof(INT32)) * headCnt);

	auto body = make_unique<UINT8[]>(bodysize);
	int pos = 0;

	CopyBuffer(body.get(), (void*)headCnt, sizeof(UINT8), pos);

	for (int i = 0; i < positions.size(); i++)
	{
		UINT8 item1 = std::get<0>(positions[i]);
		INT32 item2 = SWAP32(std::get<1>(positions[i]));

		CopyBuffer(body.get(), (void*)item1, sizeof(UINT8), pos);
		CopyBuffer(body.get(), (void*)item2, sizeof(INT32), pos);
	}

	auto pPktNext = GetPktBase(ENG_LTCT::en_write, ENG_LCPF::en_set_position_multi,
								sendingBuff.get(), 128, body.get(), bodysize);
	
	return sendingBuff;
}

unique_ptr<UINT8[]> CPhotoheadFocus::SendPktPosition_hi_res(ENG_LPGS flag)
{
	throw exception("not implement.");// 이건 스텝모터에서만 작동한다고함. 
}

unique_ptr<UINT8[]> CPhotoheadFocus::SendPktOutside_dof_status(ENG_LPGS flag)
{
	throw exception("not implement.");// 이건 스텝모터에서만 작동한다고함. 
}

unique_ptr<UINT8[]> CPhotoheadFocus::SendPktAf_gain(ENG_LPGS flag, UINT16 gain)
{
	auto sendingBuff = GetSendingBuffer();
	...
	CopyBuffer(body, (void*)gain, sizeof(UINT16), pos);
	
	auto pPktNext = GetPktBase(ENG_LPGS::en_set == flag ? ENG_LTCT::en_write : ENG_LTCT::en_read, ENG_LCPF::en_af_gain,
		sendingBuff.get(), 128, body, bodysize);

	return sendingBuff;
}

unique_ptr<UINT8[]> CPhotoheadFocus::SendPktScan_ethercat()
{
	auto sendingBuff = GetSendingBuffer();
	auto pPktNext = GetPktBase(ENG_LTCT::en_read, ENG_LCPF::en_scan_ethercat, sendingBuff.get(), 128);
	return sendingBuff;
}
