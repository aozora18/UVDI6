...
protected:
    ...
    public:
		// April 4, 2005
		//
		// This base class should have defined a virtual destructor
		// to ensure that the Cleanup function's call to operator
		// delete invoked the most derived destructor code
		// (CMclLinkedListDataNode in particular) before releasing
		// the node memory.  Without this fix in place, any object
		// placed on a list by value would not be cleaned up propertly
		// if the list was destroyed while it was not empty.
		//
		// Thanks to Robin Hilliard for bringing the leak to my attention.
		//
		virtual ~CMclLinkedListNode(){}
...
public:
    ...
    ~CMclLinkedList() {
		OutputDebugString(__TEXT("~CMclLinkedList\n"));
        Cleanup();
    };
...
public:
	...
	~CMclQueue()
	{
		OutputDebugString(__TEXT("~CMclQueue\n"));
	}
};
