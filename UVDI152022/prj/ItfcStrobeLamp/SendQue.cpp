...
CSendQue::CSendQue(UINT32 count)
{
	...
	/* -------------------------------------- */
	m_pstQue = new STG_PP_PHILHMI[count];
	ASSERT(m_pstQue);
}
...
*/
LPG_PP_PHILHMI CSendQue::PopData()
{
	/* 현재 큐가 비어 있는지 확인 */
...
/*
 ...
 retn : TRUE or FALSE
*/
BOOL CSendQue::PushData(LPG_PP_PHILHMI data)
{
	UINT32 u32RearIdx	= 0;
	LPG_PP_PHILHMI pstQue		= NULL;

	/* 현재 등록하고자 하는 큐 공간이 없는지 확인 */
	if (IsQueFull())
	{
		uvLogs_SaveLogs(ENG_EDIC::en_philhmi, ENG_LNWE::en_warning, L"The send packet buffer is full",
						WFILE, WFUNC, WLINE);
		return FALSE;
	}
	...
	/* 송신 버퍼 메모리 복사 */
	memcpy(pstQue, data, sizeof(STG_PP_PHILHMI));
