...
void CSocketSComm::SLEEP(long lmsec)
{
	Sleep(m_lSleepUnit*lmsec);
}
...
//弛 3. return		弛 return		nSize		: Q縑 厥擎 Data Size				弛
//戌式式式式式式式式式式式式式式式式式扛式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式戎
long CSocketSComm::PushSendQ(BYTE *pbyData, unsigned int nSize)
{
	::EnterCriticalSection(&m_csSendMsgQ);
...
//弛 3. return		弛 return		nSize		: Q縑憮 疏頂螞 Data Size			弛
//戌式式式式式式式式式式式式式式式式式扛式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式戎
long CSocketSComm::PopSendQ(BYTE *pbyData, unsigned int nSize)
{
	::EnterCriticalSection(&m_csSendMsgQ);
...
//弛 3. return		弛 return		nSize		: Q Size						弛
//戌式式式式式式式式式式式式式式式式式扛式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式戎
long CSocketSComm::FrontSendQ(BYTE *pbyData)
{
	::EnterCriticalSection(&m_csSendMsgQ);
	long lQSize = (long)m_pbySend->size();
	if (lQSize > 0)
...
//弛 3. return		弛 return		nSize		: Q縑 厥擎 Data Size				弛
//戌式式式式式式式式式式式式式式式式式扛式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式戎
long CSocketSComm::PushRecvQ(BYTE *pbyData, unsigned int nSize)
{
	::EnterCriticalSection(&m_csRecvMsgQ);
...
//弛 3. return		弛 return		nSize		: Q縑憮 疏頂螞 Data Size			弛
//戌式式式式式式式式式式式式式式式式式扛式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式戎
long CSocketSComm::PopRecvQ(BYTE *pbyData, unsigned int nSize)
{
	::EnterCriticalSection(&m_csRecvMsgQ);
...
//弛 3. return		弛 return		nSize		: Q Size						弛
//戌式式式式式式式式式式式式式式式式式扛式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式戎
long CSocketSComm::FrontRecvQ(BYTE *pbyData)
{
	::EnterCriticalSection(&m_csRecvMsgQ);
	long lQSize = (long)m_pbyRecv->size();
	if (lQSize > 0)
...
// Socket Config
void CSocketSComm::SetConfigErrorCheck(bool bErrChk)
{
	m_bErrorChk = bErrChk;
}

void CSocketSComm::SetConfigMaxBufSize(unsigned short unSize)
{
	m_unMaxBufSize = unSize;
}

void CSocketSComm::SetConfigPortNumber(unsigned short unPort)
{
	m_unPortNum = unPort;
}

void CSocketSComm::SetConfigIPAddress(const char *pszIPAddr)
{
	long lLen = (long)strlen(pszIPAddr);
	if (lLen < 7 || lLen > 15)
		return;

	sprintf_s(m_szIPAddress, 16, "%s", pszIPAddr);
	m_szIPAddress[lLen] = 0x00;
}

bool CSocketSComm::GetConfigErrorCheck()
{
	return m_bErrorChk;
}

unsigned short CSocketSComm::GetConfigMaxBufSize()
{
	return m_unMaxBufSize;
}

unsigned short CSocketSComm::GetConfigPortNumber()
{
	return m_unPortNum;
}

void CSocketSComm::GetConfigIPAddress(char *pszIPAddr)
{
	long lLen = (long)strlen(m_szIPAddress);
	sprintf_s(pszIPAddr, 16, "%s", m_szIPAddress);
	m_szIPAddress[lLen] = 0x00;
}

void CSocketSComm::SetFlagOpen(bool bOpen)
{
	m_bNetOpen = bOpen;
}

void CSocketSComm::SetFlagConnected(bool bConnected)
{
	m_bConnected = bConnected;
}

void CSocketSComm::SetFlagExitThread(bool bExit)
{
	m_bExitThread = bExit;
}

void CSocketSComm::SetFlagRetryConnect(bool bReconnect)
{
	m_bRetryConnect = bReconnect;
}

void CSocketSComm::SetFlagSendAble(bool bSendAble)
{
	m_bSendAble = bSendAble;
}

void CSocketSComm::SetFlagUseMasterThread(bool bUse)
{
	m_bUseMasterThread = bUse;
}

void CSocketSComm::SetFlagUseSlaveThread(bool bUse)
{
	m_bUseSlaveThread = bUse;
}

bool CSocketSComm::GetFlagOpen()
{
	return	m_bNetOpen;
}

bool CSocketSComm::GetFlagConnected()
{
	return	m_bConnected;
}

bool CSocketSComm::GetFlagExitThread()
{
	return	m_bExitThread;
}

bool CSocketSComm::GetFlagRetryConnect()
{
	return	m_bRetryConnect;
}

bool CSocketSComm::GetFlagSendAble()
{
	return	m_bSendAble;
}

bool CSocketSComm::GetFlagMasterThread()
{
	return	m_bUseMasterThread;
}

bool CSocketSComm::GetFlagSlaveThread()
{
	return	m_bUseSlaveThread;
}

bool CSocketSComm::IsConnect()
{
	bool bOpen = GetFlagOpen();
	bool bConnect = GetFlagConnected();
...
bool CSocketSComm::IsOpen()
{
	return GetFlagOpen();
}

void CSocketSComm::SendToSocket(long lSendSize, BYTE *pbyData)
{
	//int nResult = send(m_DataSocket, (char*)pbyData, lSendSize, 0);
	...
	while (!bAccept)
	{
		nResult = send(m_DataSocket, (char*)pbyData + nSendPos, static_cast<int>(nBuffSize - nSendPos), 0);
...
void CSocketSComm::RecvFromSocket()
{
	int nRecvSize = 0;
	unsigned short unMaxSize = GetConfigMaxBufSize();
	char* pszBuf = new char[unMaxSize];
	...
	//if (recv(m_DataSocket, pszBuf, unMaxSize, MSG_PEEK) <= 0)
	//{
	//	delete[] pszBuf;
	//	return;
	//}

	nRecvSize = recv(m_DataSocket, pszBuf, unMaxSize, 0);
...
bool CSocketSComm::NetOpen(const char *szAddress, unsigned short nPortNum, unsigned short nBufSize, bool bChkError)
{
	if (IsOpen() == true)
	{
		// 檜嘐 翱唸腎橫 氈朝 status縑憮朝 TRUE 奩��
	...
	m_NetAddress.sin_family = AF_INET;
// 	m_NetAddress.sin_addr.s_addr = inet_addr(szAddress);
	inet_pton(AF_INET, szAddress, &m_NetAddress.sin_addr.s_addr);
	m_NetAddress.sin_port = htons(nPortNum);
	...
	if (::bind(m_wSocket, (struct sockaddr*)&m_NetAddress, sizeof(m_NetAddress)) == SOCKET_ERROR)
	{
		closesocket(m_wSocket);
		WSACleanup();
		return false;
	...
	if (listen(m_wSocket, SOMAXCONN) == SOCKET_ERROR)
	{
		closesocket(m_wSocket);
		WSACleanup();
		return false;
...
bool CSocketSComm::NetClose(bool bRetry)
{
	if (GetFlagOpen() == false)
	{
		SetFlagConnected(false);
		return true;
	}
...
void CSocketSComm::Send(long lSize, BYTE *pbyData)
{
	if (IsConnect() == true)
	{
		if (GetFlagSendAble() == true)
...
unsigned int __stdcall CSocketSComm::ThreadEvent(void *lpParam)
{
	WSANETWORKEVENTS WorkEvents;
	SOCKADDR_IN NetAddressClinet;

	_Thread* pThis = (_Thread*)lpParam;
	WSAEVENT hEvent = WSACreateEvent();
	int nSizeOfNetAddress = sizeof(NetAddressClinet);
	...
	while (1)
	{
		...
		memset(&WorkEvents, 0x00, sizeof WorkEvents);
		WSAEventSelect(pThis->_this->m_wSocket, hEvent, FD_ACCEPT);
		WSAEnumNetworkEvents(pThis->_this->m_wSocket, hEvent, &WorkEvents);

		if ((WorkEvents.lNetworkEvents & FD_ACCEPT) == FD_ACCEPT)
		{
			pThis->_this->SetFlagConnected(true);
			SOCKET	wClient;
			if ((wClient = accept(pThis->_this->m_wSocket, (struct sockaddr*)&NetAddressClinet, &nSizeOfNetAddress)) != INVALID_SOCKET)
			{
				pThis->_this->m_DataSocket = wClient;
				pThis->_this->SetFlagSendAble(false);
				// �挫� в蹂й 蛭..
				pThis->_this->SetFlagSendAble(true);
			}
		...
		// Data Socket Event 籀葬
		memset(&WorkEvents, 0x00, sizeof WorkEvents);
		WSAEventSelect(pThis->_this->m_DataSocket, hEvent, FD_CLOSE | FD_WRITE | FD_READ);
		WSAEnumNetworkEvents(pThis->_this->m_DataSocket, hEvent, &WorkEvents);

		if ((WorkEvents.lNetworkEvents & FD_CLOSE) == FD_CLOSE)
		{
			pThis->_this->SetFlagConnected(false);

			pThis->_this->OnDisconnect();

			::EnterCriticalSection(&pThis->_this->m_csSocket);

			pThis->_this->SetFlagSendAble(false);
			closesocket(pThis->_this->m_DataSocket);
...
unsigned int __stdcall CSocketSComm::ThreadRecv(void *lpParam)
{
	_Thread* pThis = (_Thread*)lpParam;
...
unsigned int __stdcall CSocketSComm::ThreadSend(void *lpParam)
{
	_Thread* pThis = (_Thread*)lpParam;
...
unsigned int __stdcall CSocketSComm::ThreadMasterReq(void *lpParam)
{
	_Thread* pThis = (_Thread*)lpParam;
...
unsigned int __stdcall CSocketSComm::ThreadSlaveReq(void *lpParam)
{
	_Thread* pThis = (_Thread*)lpParam;
