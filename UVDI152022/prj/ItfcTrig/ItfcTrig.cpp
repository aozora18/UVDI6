...
CTrigThread					*g_pTrigThread		= NULL;		// 데이터 수집 및 감시 스레드
...
#if 0	/* 현재 DI 장비의 Trigger Board의 문제로 인해... 아래와 같이하면 Ring Lamp 동작 안됨 */
...
#else	/* Ring Lamp의 Camera 쪽 Trigger 발생 시키기 위해 어쩔수 없이 1 & 3 CH도 발생하도록 함 */
	switch (g_pstConfig->set_comn.strobe_lamp_type)
	{
	case 0x00	:	/* 조명 1 (Lamp: Coaxial) */
		switch (cam_id)
