...
inline LPTSTR ErrorMessage(DWORD dw)
{
	...
	else
	{
		...
		swprintf_s(msgBuf, 30, _T("Unknown error: %u"), dw);
		return msgBuf;
	}
...
_purecall_handler WheatyExceptionReport::m_previousPureCallHandler;
...
long WheatyExceptionReport::m_ExceptionCount;
...
WheatyExceptionReport::WheatyExceptionReport()             // Constructor
{
	...
	m_previousPureCallHandler = _set_purecall_handler(WheatyPureCallHandler);
	signal(SIGABRT, WheatySignal);
...
WheatyExceptionReport::~WheatyExceptionReport()
{
	...
	if (m_previousPureCallHandler)
		_set_purecall_handler(m_previousPureCallHandler);
	ClearSymbols();
}

unsigned int __stdcall WheatyExceptionReport::ThreadMiniDump(void *lpParam)
{
	MinidumpInfo* pThis = (MinidumpInfo*)lpParam;
	...
	swprintf_s(crash_folder_path, _T("%s\\%s"), module_folder_name, CrashFolder);
	...
	swprintf_s(m_szLogFileName, _T("%s\\%s_%02d[%u-%u_%u-%u-%u-%u].txt"),
		crash_folder_path, pos, m_ExceptionCount++, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond, systime.wMilliseconds
	);

	_tfopen_s(&m_hReportFile, m_szLogFileName, _T("wb"));

	if (m_hReportFile)
	{
		GenerateExceptionReport(pThis->pException);

		fclose(m_hReportFile);
		m_hReportFile = nullptr;
	}

	swprintf_s(m_szDumpFileName, _T("%s\\%s_%02d[%u-%u_%u-%u-%u-%u].dmp"),
		crash_folder_path, pos, m_ExceptionCount++, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond, systime.wMilliseconds
	);
	...
	if (m_hDumpFile)
	{
		...
		info.ExceptionPointers = pThis->pException;
		info.ThreadId = GetCurrentThreadId();

		MINIDUMP_USER_STREAM additionalStream = {};
		MINIDUMP_USER_STREAM_INFORMATION additionalStreamInfo = {};

		if (pThis->pException->ExceptionRecord->ExceptionCode == EXCEPTION_ASSERTION_FAILURE && pThis->pException->ExceptionRecord->NumberParameters > 0)
		{
			additionalStream.Type = CommentStreamA;
			additionalStream.Buffer = reinterpret_cast<PVOID>(pThis->pException->ExceptionRecord->ExceptionInformation[0]);
			additionalStream.BufferSize = (ULONG)strlen(reinterpret_cast<char const*>(pThis->pException->ExceptionRecord->ExceptionInformation[0])) + 1;
...
// 		HANDLE hThread = (HANDLE)_beginthreadex(NULL, 20000000, ThreadMiniDump, &info, 0, NULL);
...
void __cdecl WheatyExceptionReport::WheatyPureCallHandler()
{
	RaiseException(EXCEPTION_ACCESS_VIOLATION, 0, 0, nullptr);
}

void __cdecl WheatyExceptionReport::WheatySignal(int signal)
{
	RaiseException(EXCEPTION_ACCESS_VIOLATION, 0, 0, nullptr);
}
...
BOOL WheatyExceptionReport::_GetProcessorName(TCHAR* sProcessorName, DWORD maxcount)
{
	...
	while (iswspace(*psz))
		++psz;
	wcsncpy_s(sProcessorName, maxcount, psz, maxcount);
	return TRUE;
}
	...
	case VER_PLATFORM_WIN32_NT:
	{
		...
		if (osvi.dwMajorVersion == 10)
		{
			if (productType == VER_NT_WORKSTATION)
				wcsncat_s(szVersion, cntMax, _T("Windows 10 "), cntMax);
			else
				wcsncat_s(szVersion, cntMax, _T("Windows Server 2016 "), cntMax);
		}
		else if (osvi.dwMajorVersion == 6)
		{
			if (productType == VER_NT_WORKSTATION)
			{
				if (osvi.dwMinorVersion == 3)
					wcsncat_s(szVersion, cntMax, _T("Windows 8.1 "), cntMax);
				else if (osvi.dwMinorVersion == 2)
					wcsncat_s(szVersion, cntMax, _T("Windows 8 "), cntMax);
				else if (osvi.dwMinorVersion == 1)
					wcsncat_s(szVersion, cntMax, _T("Windows 7 "), cntMax);
				else
					wcsncat_s(szVersion, cntMax, _T("Windows Vista "), cntMax);
			}
			else if (osvi.dwMinorVersion == 3)
				wcsncat_s(szVersion, cntMax, _T("Windows Server 2012 R2 "), cntMax);
			else if (osvi.dwMinorVersion == 2)
				wcsncat_s(szVersion, cntMax, _T("Windows Server 2012 "), cntMax);
			else if (osvi.dwMinorVersion == 1)
				wcsncat_s(szVersion, cntMax, _T("Windows Server 2008 R2 "), cntMax);
			else
				wcsncat_s(szVersion, cntMax, _T("Windows Server 2008 "), cntMax);
		}
		else if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 2)
			wcsncat_s(szVersion, cntMax, _T("Microsoft Windows Server 2003 "), cntMax);
		else if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 1)
			wcsncat_s(szVersion, cntMax, _T("Microsoft Windows XP "), cntMax);
		else if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 0)
			wcsncat_s(szVersion, cntMax, _T("Microsoft Windows 2000 "), cntMax);
		else if (osvi.dwMajorVersion <= 4)
			wcsncat_s(szVersion, cntMax, _T("Microsoft Windows NT "), cntMax);
		...
		if (bVersionEx >= 0)
		{
			...
			if (productType == VER_NT_WORKSTATION)
			{
				if (osvi.dwMajorVersion == 4)
					wcsncat_s(szVersion, cntMax, _T("Workstation 4.0 "), cntMax);
				else if (suiteMask & VER_SUITE_PERSONAL)
					wcsncat_s(szVersion, cntMax, _T("Home Edition "), cntMax);
				else if (suiteMask & VER_SUITE_EMBEDDEDNT)
					wcsncat_s(szVersion, cntMax, _T("Embedded "), cntMax);
				else
					wcsncat_s(szVersion, cntMax, _T("Professional "), cntMax);
			}
			...
			else if (productType == VER_NT_SERVER)
			{
				if (osvi.dwMajorVersion == 6 || osvi.dwMajorVersion == 10)
				{
					if (suiteMask & VER_SUITE_SMALLBUSINESS_RESTRICTED)
						wcsncat_s(szVersion, cntMax, _T("Essentials "), cntMax);
					else if (suiteMask & VER_SUITE_DATACENTER)
						wcsncat_s(szVersion, cntMax, _T("Datacenter "), cntMax);
					else if (suiteMask & VER_SUITE_ENTERPRISE)
						wcsncat_s(szVersion, cntMax, _T("Enterprise "), cntMax);
					else
						wcsncat_s(szVersion, cntMax, _T("Standard "), cntMax);
				}
				else if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 2)
				{
					if (suiteMask & VER_SUITE_DATACENTER)
						wcsncat_s(szVersion, cntMax, _T("Datacenter Edition "), cntMax);
					else if (suiteMask & VER_SUITE_ENTERPRISE)
						wcsncat_s(szVersion, cntMax, _T("Enterprise Edition "), cntMax);
					else if (suiteMask == VER_SUITE_BLADE)
						wcsncat_s(szVersion, cntMax, _T("Web Edition "), cntMax);
					else
						wcsncat_s(szVersion, cntMax, _T("Standard Edition "), cntMax);
				}
				else if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 0)
				{
					if (suiteMask & VER_SUITE_DATACENTER)
						wcsncat_s(szVersion, cntMax, _T("Datacenter Server "), cntMax);
					else if (suiteMask & VER_SUITE_ENTERPRISE)
						wcsncat_s(szVersion, cntMax, _T("Advanced Server "), cntMax);
					else
						wcsncat_s(szVersion, cntMax, _T("Server "), cntMax);
				}
				else                                        // Windows NT 4.0
				{
					if (suiteMask & VER_SUITE_ENTERPRISE)
						wcsncat_s(szVersion, cntMax, _T("Server 4.0, Enterprise Edition "), cntMax);
					else
						wcsncat_s(szVersion, cntMax, _T("Server 4.0 "), cntMax);
				}
			}
		}
		...
		if (osvi.dwMajorVersion == 4 && _tcsicmp(szCSDVersion, _T("Service Pack 6")) == 0)
		{
			...
			if (lRet == ERROR_SUCCESS)
			{
				swprintf_s(wszTmp, 128, _T("Service Pack 6a (Version %d.%d, Build %d)"),
					osvi.dwMajorVersion, osvi.dwMinorVersion, osvi.dwBuildNumber & 0xFFFF);
				wcsncat_s(szVersion, cntMax, wszTmp, cntMax);
			}
			else                                            // Windows NT 4.0 prior to SP6a
			{
				swprintf_s(wszTmp, 128,  _T("%s (Version %d.%d, Build %d)"),
		...
		else                                                // Windows NT 3.51 and earlier or Windows 2000 and later
		{
			if (!_tcslen(szCSDVersion))
				swprintf_s(wszTmp, 128,  _T("(Version %d.%d, Build %d)"),
					osvi.dwMajorVersion, osvi.dwMinorVersion, osvi.dwBuildNumber & 0xFFFF);
			else
				swprintf_s(wszTmp, 128,  _T("%s (Version %d.%d, Build %d)"),
					szCSDVersion, osvi.dwMajorVersion, osvi.dwMinorVersion, osvi.dwBuildNumber & 0xFFFF);
	...
	PEXCEPTION_POINTERS pExceptionInfo)
{
	__try
	{
		...
		if (pExceptionRecord->ExceptionCode == 0xE06D7363 && pExceptionRecord->NumberParameters >= 2)
		{
			...
			auto resolveExceptionRVA = [pExceptionRecord](int rva) -> DWORD_PTR
			{
	...
	PVOID addr, PTSTR szModule, DWORD len, DWORD& section, DWORD_PTR& offset)
{
		...
		i++, pSection++)
	{
		...
		DWORD_PTR sectionEnd = sectionStart
			+ DWORD_PTR(((((pSection->SizeOfRawData) > (pSection->Misc.VirtualSize)) ? (pSection->SizeOfRawData) : (pSection->Misc.VirtualSize))));
	...
	bool bWriteVariables, HANDLE pThreadHandle)                                      // true if local/params should be output
{
	...
	for (;;)
	{
		...
		{
			USES_CONVERSION;
			Log(_T("  %s line %u"), A2T(lineInfo.FileName), lineInfo.LineNumber);
		}
	...
	bool logChildren)
{
		...
		&pwszTypeName))
	{
		...
		wcstombs_s(&getVal, buffer, pwszTypeName, sizeof(buffer));
	...
	size_t countOverride)
{
	__try
	{
		...
		case btChar:
		{
			...
			if (length > bufferSize - 6)
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "\"%.*s...\"", (DWORD)(bufferSize - 6), (char*)pAddress);
			else
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "\"%.*s\"", (DWORD)length, (char*)pAddress);
			break;
		}
		case btStdString:
		{
			...
			if (value->length() > bufferSize - 6)
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "\"%.*s...\"", (DWORD)(bufferSize - 6), value->c_str());
			else
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "\"%s\"", value->c_str());
			break;
		}
		default:
			// Format appropriately (assuming it's a 1, 2, or 4 bytes (!!!)
			if (length == 1)
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%X", *(PBYTE)pAddress);
			else if (length == 2)
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%X", *(PWORD)pAddress);
			else if (length == 4)
			{
				if (basicType == btFloat)
					pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "%f", *(PFLOAT)pAddress);
				else
					pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%X", *(PDWORD)pAddress);
			}
			else if (length == 8)
			{
				if (basicType == btFloat)
				{
					pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "%f",
						*(double *)pAddress);
				}
				else
					pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%I64X",
						*(DWORD64*)pAddress);
			}
			else
			{
#if _WIN64
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%I64X", (DWORD64)pAddress);
#else
				pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%X", (DWORD)pAddress);
#endif
			}
			break;
		}
	...
	{
#if _WIN64
		pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%I64X <Unable to read memory>", (DWORD64)pAddress);
#else
		pszCurrBuffer += sprintf_s(pszCurrBuffer, bufferSize, "0x%X <Unable to read memory>", (DWORD)pAddress);
#endif
	}
}
...
{
	TCHAR chArrDebug[2048];
	va_list argptr;
	va_start(argptr, format);
	int retValue = _vftprintf(m_hReportFile, format, argptr);
	vswprintf_s(chArrDebug, 2048, format, argptr);
	va_end(argptr);

	OutputDebugString(chArrDebug);
