...
VOID CDlgMmpm::OnMouseMove(UINT flags, CPoint point)
{
	...
	//	UINT32 u32AoiW	= uvEng_GetConfig()->set_cams.aoi_size[0];
	//	UINT32 u32AoiH	= uvEng_GetConfig()->set_cams.aoi_size[1];
	//	DOUBLE dbRate;
	//	CRect rPic;
	//
	//	/* 마우스 좌측 버튼을 클릭여부 확인 */
	//	if (!m_u8LeftClick)	return;
	//
	//	/* 이미지 출력 영역 얻기 */
	//	m_pic_ctl[eMMPM_PIC_VIEW].GetWindowRect(rPic);
	//	ScreenToClient(rPic);
	...
	//	if ((DOUBLE(rPic.Width()) / DOUBLE(u32AoiW)) > (DOUBLE(rPic.Height()) / DOUBLE(u32AoiH)))
	//	{
	//		/* <세로> 비율 기준으로 영상 확대 or 축소 진행 */
	//		dbRate	= DOUBLE(rPic.Height()) / DOUBLE(u32AoiH);
	//	}
	//	/* 세로 크기 비율이 가로 크기 비율보다 큰 경우 */
	//	else
	//	{
	//		/* <가로> 비율 기준으로 영상 확대 or 축소 진행 */
	//		dbRate	= DOUBLE(rPic.Width()) / DOUBLE(u32AoiW);
	//	}
	//#if 0
	//	/* 마우스 누른 위치가 이미지 영역인지 여부 확인 */
	//	if (rPic.PtInRect(point))
	//#endif
	//	{
	//		if (m_pstGrab)
	...
	//			m_pstGrab->mark_cent_px_y	= m_pstGrab->grab_height / 2.0f;
	//			/* Point X/Y 위치가 rPic의 중심에서 얼마만큼 떨어졌는지 계산 */
	//			m_pstGrab->mark_cent_px_x	+= ((rPic.left + rPic.right) / 2.0f - point.x) / dbRate;
	//			m_pstGrab->mark_cent_px_y	+= ((rPic.top + rPic.bottom) / 2.0f - point.y) / dbRate;
	//		}
	//		/* 이동한 위치 값 적용 */
	//		MoveCenter(0x00);
	//	}
