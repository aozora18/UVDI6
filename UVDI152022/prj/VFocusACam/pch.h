...
{
	TCHAR			file[MAX_PATH_LEN];
#if 1
	UINT8			set_value[COLLECT_DATA_COUNT];			/* 0x00 : strength, 0x01 : length, 0x02 : Feret, 0x03 : Diameter */
	UINT8			u8_reserved[5];
#endif
	INT32			z_pos;									/* 현재 Z Axis의 높이 값 (0.1 um or 100 nm) */
	INT32			find_count;								/* Edge or Match 등 검색된 개수 */

	/* 평균, 최대, 최소 값*/
	DOUBLE			avg_value[COLLECT_DATA_COUNT];			/* 0x00 : strength, 0x01 : length, 0x02 : Diameter */
	DOUBLE			max_value[COLLECT_DATA_COUNT];			/* 0x00 : strength, 0x01 : length, 0x02 : Diameter */
	DOUBLE			min_value[COLLECT_DATA_COUNT];			/* 0x00 : strength, 0x01 : length, 0x02 : Diameter */
	DOUBLE			std_value[COLLECT_DATA_COUNT];			/* 0x00 : strength, 0x01 : length, 0x02 : Diameter */
	/* 매번 측정된 값 */
	CArray <DOUBLE, DOUBLE>	arrValue[COLLECT_DATA_COUNT];	/* 0x00 : strength, 0x01 : length, 0x02 : Diameter */
...
public:
	...
	/* 
		설명 : 현재 등록된 값 중에 가장 큰 값과 작은 값의 차이
		변수 : type	- [in]  0x00 : strength, 0x01 : length, 0x02 : feret, 0x03 : diameter
		반환 : 차이 값 반환
	*/
	...
	/*
		설명 : LPM_ZAAL 구조체 포인터 자료 비교 함수
		변수 : elem1	- [in]  비교하기 위한 Strength 값
			elem2	- [in]  비교하기 위한 Strength 값
		반환 : elem1 - elem2인 경우, 오름차순 정렬
	...
	*/
	...
	/*
		설명 : 수집된 엣지 강도 및 길이들의 평균 값 구하기 (개인 알고리즘)
		변수 : None
		반환 : 평균 강도 값 반환
	*/
	VOID CalcAverage()
	...
	/*
		설명 : 수집된 엣지 강도 및 길이들의 평균 값 구하기 (정규 분포 알고리즘을 통해 이상치 값 제거)
		변수 : rate	- [in]  표준 편차에 얼마 값을 곱할지 개수 즉,
	...
	*/
	VOID CalcAverageEx(DOUBLE rate)
	{
		...
		/* 각 항목 별 (Strength, Length, Feret) 총합 / 평균 구하기 */
