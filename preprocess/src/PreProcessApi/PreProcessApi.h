...
#define __PREPROCESS_API_H__
    ...
    #endif
#else
    #define PP_API // GCC always exports unless explicitily hidden.
    #ifndef NO_ARTWORK_CONVERSION
        #define NO_ARTWORK_CONVERSION
    #endif
#endif
 ...
 **********************************************************************************************************************/
  ...
  /**********************************************************************************************************************
   ** InitBitmap - Same as Init but with input and output as parameters.
   **
...
/**********************************************************************************************************************
 ** GetProductName - Read out the product name of the API DLL.
 **
 ** str	- string buffer owned by the caller, version information will be stored to this buffer
 ** len - length of the string buffer
 **
 ** Return: PPResult. TEXT_TOO_LONG_ERROR if info text is longer than callers string buffer size (len parameter).
 **********************************************************************************************************************/
  PP_API int GetProductName(char* str, int len);
...
/**********************************************************************************************************************
 ** Smoothing - Enable smoothing to get smoother edges and to reduce effects of pixel snapping. Obtaining somewhat higher 
 **			 resolution at the cost of some longer pre-processing time.
 **
 ** Set:
 **		val - Bool, true=smoothing enabled
 **
 ** Is:
 **		Return: 0=false, 1=true (smoothing enabled)
 **********************************************************************************************************************/
  PP_API void SetSmoothing(bool val);
  PP_API int IsSmoothing();
 ...
 **					segment. A value 3 degrees which would generate 121 (120 + 1) vertices per circle. A value
 ...
 **					Default and recommended is set to 45, and determin the error correction with ChordError.
 **
 ** Set:
 ...
 **				 0.001 mm ChordError in combination with 45 deg ArcResolution
 **
 ** Set:
...
/**********************************************************************************************************************
 ** MTC - Enable Machine Tolerance Compensation (MTC) mode
 **
 ** Set:
 **		val - false=MTC (default), true=Enable MTC.
 **
 ** Is:
 **		Return: 0=false, 1=true
 **********************************************************************************************************************/
  PP_API void SetMTC(bool val);
  PP_API int IsMTC();
